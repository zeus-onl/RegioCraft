...
- **Grounded edit instructions (`identity_provider = krea2edit`).** Give a region both a reference image *and* its own `prompt` text (e.g. "give him a hooked nose") and RegioCraft grounds that instruction on the reference image via Qwen3-VL, using the exact template the `krea2_identity_edit` LoRA was trained on, then routes it through `comfyui-krea2edit`'s full diffusion forward. No extra toggle or dropdown — a region with only a reference image (no prompt) still does plain identity/face lock exactly as before; ref_image + prompt together *is* the signal that switches this on. One combined instruction applies per model call — if several regions carry both at once, their texts are concatenated rather than applied per-region separately.
...
**Grounded edit instructions**
- `identity_provider = krea2edit` regions that have BOTH a reference image and their own `prompt` text now ground that text on the reference image via Qwen3-VL, using the same template `comfyui-krea2edit`'s `Krea2EditGroundedEncode` node uses (training-matched), and pass it through as the actual model conditioning for that call -- turns identity lock into a real edit instruction ("give him a hooked nose") instead of just holding the face steady
- Detection is automatic from the two fields already on the region row -- no new widget or dropdown. `ref_image` alone = old behavior (pure identity/face lock), `ref_image` + `prompt` = grounded edit
- Regions using plain LoRA masking (no ref_image) are completely unaffected -- unlimited character-LoRA regions still work exactly as before
- Limit: one combined edit instruction per model call (the underlying `krea2_edit_forward` engine runs the diffusion model once per call, not once per region) -- if multiple active regions have both ref_image and prompt at once, their texts are concatenated into a single instruction rather than applied to each region separately
